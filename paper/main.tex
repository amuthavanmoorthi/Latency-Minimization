...
policy degrades by $13.7\times$ to 25.2~ms. The DT-BC training pipeline requires $22\times$ fewer
environment interactions than deep deterministic policy gradient
(DDPG), with zero on-line URLLC-violating exploration steps.
\end{abstract}
...
policy training entirely offline
\cite{paul2024active,kurma2024ris,masaracchia2025dt}. DTs empower
...
\cite{kurma2024activeris,lestari2024icc}, whilst eliminating the
exploratory safety violations inherent to on-line learning approaches.
...
Joint scheduling surveys \cite{durisi2016toward} and DT-assisted
task-offloading frameworks \cite{popovski2018slicing} have established
the need for integrated communication-computation designs in
next-generation IoT networks. With the increasing demand for
computation-intensive tasks within strict latency budgets, MEC has
emerged as a compelling solution for efficient task processing in
URLLC systems \cite{mao2017survey}. At the same time, DT technology provides
practical features that enable organisations to meet the stringent
requirements of high reliability and low latency; the incorporation
of DT with RIS-aided MEC for URLLC has been investigated in
\cite{kurma2024ris}, where a digital-twin-driven architecture
achieves up to 60\% lower transmission delay compared to schemes
without RIS. The role of digital twins in 6G-based URLLC systems,
including research challenges and future directions, is surveyed in
\cite{masaracchia2025dt}. The interplay between pilot overhead and
FBL payload rate in massive-MIMO URLLC was analysed in
\cite{ren2020joint}, where it was shown that CSI errors couple
multiplicatively with the short-packet rate penalty. Model-free deep
reinforcement learning (DRL) approaches have been applied to RIS
...
that characterise millimetre-wave URLLC deployments. Active RIS
integrated with digital twins for URLLC IoT networks has been studied
in \cite{kurma2024activeris}, which compares fully-connected and
sub-connected architectures; resource allocation and beamforming for
active STAR-RIS-assisted wireless-powered MEC is investigated in
\cite{lestari2025fbl}. The conference precursor of the present
work~\cite{lestari2024icc} first demonstrated the potential of active
STAR-RIS in DT-based URLLC IoT, while the extended journal version
\cite{paul2024active} proposes a complete framework with FBL rate
modelling and partial MEC offloading but assumes a single-antenna BS
and perfect CSI, and employs a fractional-programming optimiser that
degrades under realistic channel estimation noise.
Table~\ref{tab:relwork} offers a comprehensive investigation of the
existing works and highlights the contributions of the proposed
framework.
...
Consider the scenario illustrated in Fig.~\ref{fig:system}.
...
estimation. Using MMSE pilot-aided channel estimation \cite{hassibi2003how},
the residual estimation error of any channel $\vect{h}$ is modelled
as an independent complex Gaussian perturbation
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:phistar}
\end{equation}
The resulting received SNR satisfies the upper bound
\begin{equation}
  \gamma_{z,k}^{\mathrm{coh}} \le
  \frac{M P A_{\max}^2 N^2
  \beta_{\mathrm{BR}}\beta_{\mathrm{RU}_z}}{2\sigma^2},
  \label{eq:snr_bound}
\end{equation}
a $2MN^2$-fold gain over the random-phase baseline.
\end{proposition}
\begin{proof}
...
\end{proposition}
\begin{proof}
Setting $L_{z,k}^{\mathrm{off}}\!=\!L_{z,k}^{\mathrm{loc}}$
in \eqref{eq:latuser} gives the linear equation
$\rho_k(1/R_{z,k}+C_{\mathrm{MEC}}/f_{\mathrm{MEC}}) =
(1-\rho_k)C_{\mathrm{loc}}/f_{\mathrm{dev}}$; solving for
$\rho_k$ directly yields \eqref{eq:rhostar}. Since
$L_{z,k}^{\mathrm{off}}$ is increasing and
$L_{z,k}^{\mathrm{loc}}$ is decreasing in $\rho_k$, the
minimax $\max(\cdot)$ is uniquely minimised at their
intersection. Boundary solutions $\rho_k\!=\!0$ or
$\rho_k\!=\!1$ arise only in degenerate limits that do not
occur in our parameter regime.
\end{proof}
...
since the objective is monotone decreasing in $P$
from \eqref{eq:snr} and \eqref{eq:fbl}); the last $K$ entries are the
...
accurately. A smooth $\ell_1$ loss \cite{huber1964robust} is preferred over
...
evaluates the closed-form oracle \eqref{eq:astar} on the
...
at $P\!\approx\!12\,\text{dBm}$ and saturates near $1.75\,\text{ms}$
...
validating the theoretical framework of Section~\ref{sec:method}.
...
This paper proposed a digital-twin-assisted behavioural cloning
(DT-BC) framework for joint active STAR-RIS beamforming, task
offloading, and transmit-power optimization in multi-antenna URLLC
networks under imperfect CSI. Leveraging dual-view channel access
inside the digital twin, a closed-form per-zone oracle generates
optimal (phase-shift, offloading-ratio) labels from perfect
channel state information, while a residual-MLP policy is distilled
from paired noisy observations via supervised imitation learning.
Numerical results demonstrated that DT-BC achieves
$\approx\!1.76\,\text{ms}$ mean end-to-end latency with
$100\%$ URLLC constraint satisfaction, outperforming DDPG,
random, and no-RIS baselines across all SNR, element-count,
user-load, and fading-severity regimes. Crucially, the
implicit denoising induced by the informational gap between
oracle and policy yields $13.7\!\times$ robustness over the
closed-form baseline at $\sigma_e\!=\!0.15$, while the
supervised training pipeline requires $22\!\times$ fewer
environment interactions than DDPG and completes inference
in under $0.3\,\text{ms}$. These findings confirm that the
dual-view digital twin paradigm is an effective and
computationally efficient strategy for deploying robust,
latency-optimal resource allocation in next-generation
active STAR-RIS URLLC systems.
